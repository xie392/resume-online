...
const Template = () => {
  ...
  return (
    <div
      className="text-inherit flex flex-col gap-y-8"
      ...
      <div className="flex">
        ...
        >
          <div className="flex gap-x-1">
            ...
            {baseInfo.phone && (
              <span className="pr-1 border-r border-r-black/20 last:border-r-transparent">
                {baseInfo.phone}
              </span>
            )}

            {baseInfo.email && <span className="pr-1 ">{baseInfo.email}</span>}
          </div>
      ...
      <div className="flex flex-col">
        ...
        {educationExp?.map((item) => (
          <div className="flex flex-col gap-y-2" key={item.school}>
            ...
            <div
              className="rich-text"
              dangerouslySetInnerHTML={{ __html: item.schoolExperience ?? "" }}
            />
          </div>
        ))}
      </div>
